...
/**
 * Liquid Plasma — a WebGL2 fluid-frosting background. A ping-pong velocity field
 * is advected, diffused and pushed by the cursor; the render pass lays dead-straight
 * rainbow stripes over a swirled, softly-lit "frosting" relief. Falls back to a static
 * CSS gradient when WebGL2 is unavailable.
 */
export const LiquidPlasma = ({
  ...
  // Live-read every knob so dragging a slider never forces a GL rebuild.
  ...
  useEffect(() => {
    ...
    if (!gl) {
      // No WebGL2 — the CSS gradient fallback stays visible.
    ...
    // full-screen quad (single covering triangle)
    ...
    const target = (w: number, h: number): Field => {
    ...
    const frame = (now: number) => {
      ...
      // cursor velocity (uv/sec), clamped
      ...
      const idle = 0.0 // displacement stays put until the cursor touches it
