...
import { DOCS, docPath } from "@/docs/registry"

/**
 * Docs left nav — identical look/behaviour to the components Sidebar, driven
 * by the docs registry. Active-item styling, mobile drawer, and toggle all
 * mirror src/components/ui/Siebar.tsx.
 */
export default function DocsSidebar({ onToggle }: { onToggle?: (isOpen: boolean) => void }) {
    ...
    const menuItems = DOCS.map((d) => ({ id: d.slug, label: d.title, to: docPath(d.slug) }))

    const isActiveItem = (to: string) => location.pathname.replace(/\/$/, "") === to
                ...
                <div className="flex-1 overflow-y-auto py-4 sidebar-scroll">
                    ...
                    <div className="space-y-1 px-3">
                        {menuItems.map((item) => (
                            <button
                                ...
                                ${isActiveItem(item.to) ? "bg-gradient-to-r from-purple-700/20 to-blue-600/10 text-text-primary"
                  ...
                  `}
                            >
                                ...
                                <span className={`${isActiveItem(item.to) ? "font-medium" : ""}"`}>{item.label}</span>

                                {isActiveItem(item.to) && <div className="absolute right-3 w-1.5 h-1.5 rounded-full bg-blue-600" />}
                            </button>
                        ))}
                    </div>
                </div>
